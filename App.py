...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 67.0 (Visual Compare)", page_icon="🧬", layout="wide")
...
<style>
    ...
    .prediction-text { font-weight: bold; color: #2e7d32; padding: 10px; background-color: #f1f8e9; border-radius: 8px; border: 1px solid #c5e1a5; margin-bottom: 10px; text-align: center; }
    .compare-label { font-size: 0.8rem; color: #666; text-align: center; margin-bottom: 2px; font-weight: bold; }
...
# --- VARIABLES PARA COMPARATIVA VISUAL ---
...
def predecir_cierre_texto():
    ...
    if reduccion_total <= 0:
        return "⚠️ Sin mejoría detectada. Revisar plan."
    
    reduccion_promedio = reduccion_total / (len(areas) - 1)
    if reduccion_promedio <= 0: return "⚠️ Estancamiento."
    
    dias_restantes = areas[-1] / reduccion_promedio
    dias_estimados = int(dias_restantes * 1.5)
    
    return f"📉 Tendencia Positiva: Cierre estimado en <b>{dias_estimados}-{dias_estimados+7} días</b>."
        ...
        if st.button("🚀 ANALIZAR", type="primary"):
            ...
            with st.spinner(f"🧠 Analizando {modo}..."):
                try:
                        ...
                        if "video" in label: pass 
                            ...
                            if ("Heridas" in modo or "Dermatología" in modo):
                                if "prev" in label:
                                    ...
                                    st.session_state.img_previo = img_pil # Guardar para comparativa
                                else:
                                    ...
                                    st.session_state.img_actual = img_medida # Guardar para comparativa
                    ...
                    if mostrar_imagenes:
                        # Si hay imagen, la comparativa la hace la columna de la derecha ahora
                        # Pero podemos mostrar la térmica aquí como extra
...
# --- COLUMNA 3 (MODIFICADA: SIN GRÁFICA, SOLO COMPARATIVA Y TEXTO) ---
with col_right:
    with st.expander("🔮 Evolución", expanded=True):
        if len(st.session_state.historial_evolucion) > 0:
            # 1. PREDICCIÓN TEXTUAL
            pred_text = predecir_cierre_texto()
            st.markdown(f'<div class="prediction-text">{pred_text}</div>', unsafe_allow_html=True)
            
            # 2. COMPARATIVA VISUAL (Si hay imágenes)
            if st.session_state.img_previo and st.session_state.img_actual:
                st.markdown('<div class="compare-label">ANTES vs AHORA</div>', unsafe_allow_html=True)
                c_prev, c_curr = st.columns(2)
                with c_prev: st.image(st.session_state.img_previo, caption="Inicio", use_container_width=True)
                with c_curr: st.image(st.session_state.img_actual, caption="Actual", use_container_width=True)
            elif st.session_state.img_actual:
                st.image(st.session_state.img_actual, caption="Estado Actual", use_container_width=True)
            else:
                st.caption("Sube fotos (Previa + Actual) para ver la comparativa aquí.")
        else:
