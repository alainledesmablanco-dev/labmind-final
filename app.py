    ...
    if btn_analizar:
        ...
        with st.spinner("Procesando datos (Razonamiento en Cadena activado)..."):
            try:
                ...
                # --- NUEVO: LECTURA DE PROTOCOLO DE HERIDAS DESDE UNA IMAGEN JPG ---
                ...
                if modo == "🩹 Heridas / Úlceras":
                    try:
                        if os.path.exists("protocolo.jpg"):
                            ...
                            con.append(img_protocolo) # Adjuntamos la imagen del protocolo a la IA
                            st.toast("📜 Protocolo visual adjuntado a la IA.")
                            instruccion_extra_heridas = "- INSTRUCCIÓN CRÍTICA ENFERMERÍA: Entre las imágenes adjuntas, hay una que contiene el texto/flujograma del PROTOCOLO DE HERIDAS INSTITUCIONAL. Debes leer esa imagen y usarla como tu guía EXCLUSIVA para recomendar curas y apósitos. IMPORTANTE: No confundas la imagen del protocolo con la imagen de la herida del paciente."
                    except Exception as e:
