...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 65.0 (Global State)", page_icon="🧬", layout="wide")
...
# Inicialización de Estado (Solo una vez)
...
# Leemos cookies al inicio y las metemos en session_state para usarlas globalmente
if "pref_cargadas" not in st.session_state:
    c_mon = cookie_manager.get("pref_moneda")
    c_vis = cookie_manager.get("pref_visual")
    st.session_state.global_moneda = True if c_mon == "True" else False
    st.session_state.global_visual = True if c_vis == "True" else False
    st.session_state.pref_cargadas = True

# Callbacks para guardar cambios (Evita race conditions)
def save_pref_moneda():
    cookie_manager.set("pref_moneda", str(st.session_state.chk_moneda_val), expires_at=datetime.datetime.now()+datetime.timedelta(days=30))
    st.session_state.global_moneda = st.session_state.chk_moneda_val

def save_pref_visual():
    cookie_manager.set("pref_visual", str(st.session_state.chk_visual_val), expires_at=datetime.datetime.now()+datetime.timedelta(days=30))
    st.session_state.global_visual = st.session_state.chk_visual_val
...
with col_center:
    ...
    with tab_analisis:
        ...
        if modo == "🧩 Integral (Analizar Todo)":
            ...
            # --- CHECKBOX GLOBAL (ESTABLE) ---
            mostrar_imagenes = st.checkbox("👁️ Mostrar Análisis Visual", value=st.session_state.global_visual, key="chk_visual_val", on_change=save_pref_visual)
            
            # Selector fuente estable
            fuente_label = st.radio("Fuente:", ["📁 Archivo", "📸 WebCam"], horizontal=True, label_visibility="collapsed", key="rad_source_int")
            ...
            # --- CHECKBOXES GLOBALES (ESTABLES) ---
            # Al usar un key fijo "chk_moneda_val" y guardar en session_state, no se duplican.
            usar_moneda = st.checkbox("🪙 Usar moneda de 1€ para medir", value=st.session_state.global_moneda, key="chk_moneda_val", on_change=save_pref_moneda)
            mostrar_imagenes = st.checkbox("👁️ Mostrar Análisis Visual (Biofilm/Térmica)", value=st.session_state.global_visual, key="chk_visual_val", on_change=save_pref_visual)
            ...
            # --- KEY FIJA ---
            fuente_label = st.radio("Fuente:", ["📁 Archivo", "📸 WebCam"], horizontal=True, label_visibility="collapsed", key="rad_source_wounds")
        ...
        if st.button("🚀 ANALIZAR", type="primary"):
            ...
            with st.spinner(f"🧠 Analizando {modo}..."):
                try:
                    ...
                    # --- PROMPT MEJORADO: NO REPETIR TEXTO, SOLO MARCAS EN CURA ---
                    prompt = f"""
