...
def segmentar_herida_sam_v2(img_pil):
    """Segmentación de úlceras con re-intento de encuadre"""
    try:
...
with col_l:
    ...
    # --- AUTO-SELECCIÓN DE GEMINI 3 FLASH PREVIEW ---
    idx_defecto = 0
...
with col_c:
    ...
    # --- AÑADIDO: Contexto "Residencia" ---
    contexto = st.selectbox("🏥 Contexto:", ["Urgencias", "Hospitalización", "UCI", "Residencia", "Domicilio"])
    ...
    if modo == "📚 Agente Investigador (PubMed)":
        ...
        # --- AÑADIDO: Expanders cerrados para Notas y Voz ---
    ...
    with c2:
        if st.button("🔄 NUEVO", type="secondary", use_container_width=True):
            for k in ["resultado_analisis", "img_marcada", "video_bytes", "chat_messages"]: 
    ...
    if btn_analizar:
        ...
        with st.spinner("Procesando datos (Razonamiento en Cadena activado)..."):
            try:
                ...
                # --- PROMPT MAESTRO (CON RAZONAMIENTO EN CADENA) ---
                if sam_utilizado:
                    instruccion_bbox = "INSTRUCCIÓN: La imagen ya ha sido segmentada milimétricamente. NO devuelvas BBOX."
                else:
                    instruccion_bbox = "INSTRUCCIÓN OBLIGATORIA: Si detectas patología, DEBES imprimir sus coordenadas exactas de 0 a 1000. Formato: BBOX: [ymin, xmin, ymax, xmax] LABEL: TuTexto. (Ejemplo: BBOX: [150, 200, 450, 600] LABEL: Fractura)."
                ...
                Usuario (Notas): "{notas}"
                Datos Técnicos adicionales: {txt_docs[:5000]}
                ...
                4. JUICIO CLÍNICO: Emite el diagnóstico basado estrictamente en la evidencia.
                ...
                <details class="diagnosis-box" open><summary>🚨 HALLAZGOS Y RAZONAMIENTO</summary><p><b>[Diagnóstico y Zona]</b>. [Aquí describe tu análisis estructurado]</p></details>
                <details class="action-box" open><summary>⚡ ACCIÓN INMEDIATA</summary><p>[Plan de actuación]</p></details>
                <details class="{'pubmed-box' if 'PubMed' in modo else 'material-box'}" open><summary>🛠️ TRATAMIENTO Y SEGUIMIENTO</summary><p>[Desarrollo]</p></details>
                """
                ...
                if imagen_para_visor and not sam_utilizado:
                    im_m, clean_t, det = extraer_y_dibujar_bboxes(raw_txt, imagen_para_visor)
                    st.session_state.img_marcada = im_m if det else imagen_para_visor
                    raw_txt = clean_t
