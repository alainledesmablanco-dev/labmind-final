...
with col_l:
    ...
    st.session_state.modelo_seleccionado = st.selectbox("Versión de Gemini:", st.session_state.modelos_disponibles, index=0)
...
with col_c:
    ...
    modo = st.selectbox("Especialidad:", ["✨ Autodetectar", "🩹 Heridas / Úlceras", "🦇 Ecografía / POCUS", "📈 ECG", "💀 RX/TAC", "🧴 Dermatología"])
    contexto = st.selectbox("🏥 Contexto:", ["Urgencias", "Hospitalización", "UCI", "Domicilio"])
    
    fs = st.file_uploader("Archivos Clínicos:", type=['jpg','png','pdf','mp4','mov'], accept_multiple_files=True)
    notas = st.text_area("Notas Clínicas:", placeholder="Describe el caso o especifica detalles relevantes...")
        ...
        with st.spinner("Procesando datos (Razonamiento en Cadena activado)..."):
            try:
                ...
                if sam_utilizado:
                    ...
                    instruccion_bbox = "INSTRUCCIÓN OBLIGATORIA: Si detectas patología, DEBES imprimir sus coordenadas exactas de 0 a 1000. Formato: BBOX: [ymin, xmin, ymax, xmax] LABEL: TuTexto."
