...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 29.0 (Dynamic UI)", page_icon="🧬", layout="wide")
...
<style>
    ...
    /* Ocultar textos por defecto del uploader para limpiar la UI */
    [data-testid='stFileUploaderDropzone'] div div span { display: none; }
    [data-testid='stFileUploaderDropzone'] div div::after { content: "📂 Toca para subir archivo"; font-size: 1rem; color: #555; display: block; }
</style>
...
st.title("🩺 LabMind 29.0")
col_left, col_center, col_right = st.columns([1, 2, 1])

# --- COLUMNA 1: MAPA CORPORAL ---
with col_left:
    st.subheader("📍 Mapa Interactivo")
    # Imagen de respaldo ultra estable (Wikimedia)
    body_url = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/Human_body_features.svg/280px-Human_body_features.svg.png"
    
    st.caption("Toca la zona afectada:")
    value = streamlit_image_coordinates(body_url, key="body", height=400)
    
    if value:
        st.session_state.punto_cuerpo = value
        st.success("✅ Zona Registrada")
    else:
        st.info("👆 Si la imagen no carga, usa el selector abajo:")
    
    # Selector de respaldo por si falla la imagen
    zona_backup = st.selectbox("Zona Manual:", ["N/A", "Talón", "Sacro", "Pie", "Pierna", "Brazo", "Tórax", "Cabeza"])
    if not value and zona_backup != "N/A":
         st.session_state.punto_cuerpo = {"x": 0, "y": 0, "label": zona_backup}

# --- COLUMNA 2: NÚCLEO CENTRAL (LÓGICA DINÁMICA) ---
with col_center:
    ...
    # --- INTERFAZ DINÁMICA (SOLO MUESTRA LO NECESARIO) ---
    archivos = []
    meds_files = None
    tests_files = None
    
    # 1. CASO FARMACIA (SOLO FARMACIA)
    if modo == "💊 Farmacia (Interacciones)":
        st.info("💊 **Modo Farmacia Activado**")
        st.write("Saca foto al **listado de prescripción médica** o a la **caja de la pastilla** en domicilio.")
        meds_files = st.file_uploader("Subir Receta/Caja", accept_multiple_files=True, key="pharma_only")
    
    # 2. CASO RX/TAC (SOLO RADIOLOGÍA)
    elif modo == "💀 RX / TAC / RMN (Imagen)":
        st.info("💀 **Modo Radiología Activado**")
        st.write("Sube video (scroll) de TAC/RMN o foto de Placa RX.")
        if fs := st.file_uploader("Video/Foto RX", type=['jpg','png','mp4','mov'], accept_multiple_files=True, key="rx_only"):
            for f in fs:
                ftype = "video" if "video" in f.type else "img"
                archivos.append((ftype, f))
                
    # 3. CASO ECG (SOLO CARDIOLOGÍA)
    elif modo == "📈 ECG (Cardiología)":
        st.info("📈 **Modo Cardiología Activado**")
        st.write("Sube foto del Electrocardiograma (Tira o Informe).")
        if fs := st.file_uploader("Foto ECG", type=['jpg','png','pdf'], accept_multiple_files=True, key="ecg_only"):
            for f in fs: archivos.append(("img", f))

    # 4. CASO INFORMES (SOLO DOCUMENTOS)
    elif modo == "📂 Analizar Informes":
        st.info("📂 **Modo Gestión Documental Activado**")
        st.write("Sube informes de alta, analíticas o historia clínica.")
        tests_files = st.file_uploader("Documentos PDF/Foto", accept_multiple_files=True, key="reports_only")

    # 5. CASO CLÍNICO VISUAL (HERIDAS, DERMA, INTEGRAL)
    else:
        st.info(f"📸 **Modo {modo} Activado**")
        
        # Opcional: Contexto extra solo si se quiere
        with st.expander("➕ Adjuntar Fármacos o Analíticas (Opcional)", expanded=False):
            c1, c2 = st.columns(2)
        ...
        st.write("📸 **Sube Foto/Video de la Lesión:**")
        ...
        if fuente == "📸 WebCam":
            ...
            if fs := st.file_uploader("Subir Archivo", type=['jpg','png','mp4','mov'], accept_multiple_files=True, key="std_main"):
                for f in fs:
                    ftype = "video" if "video" in f.type else "img"
                    archivos.append((ftype, f))
    ...
    if st.button("🚀 ANALIZAR", type="primary"):
        ...
        with st.spinner(f"🧠 Procesando {modo}..."):
            try:
                ...
                if st.session_state.punto_cuerpo:
                    coord_txt = f"Zona cuerpo: {st.session_state.punto_cuerpo}"
                ...
                if "Farmacia" in modo:
                    prompt += " IDENTIFICA MEDICAMENTOS (Caja/Receta). Busca interacciones, duplicidades y alergias."
                elif "RX" in modo:
                    prompt += " RADIOLOGÍA. Describe hallazgos en imagen/video (fracturas, consolidaciones)."
                elif "ECG" in modo:
                    prompt += " CARDIOLOGÍA. Analiza ritmo, ST, QT. Busca arritmias."
                elif "Informes" in modo:
                    prompt += " RESUME HISTORIA. Extrae antecedentes y plan."
                elif "Heridas" in modo:
                    prompt += " HERIDAS. Analiza térmica/biofilm. Genera HTML barra tejidos (sin markdown)."
                else:
                    prompt += " INTEGRAL. Cruza todo."
                ...
                * **🚨 DIAGNÓSTICO:**
                ...
                ### 📊 Análisis Visual
                [Si es herida, HTML puro de barra]
                ...
                """
