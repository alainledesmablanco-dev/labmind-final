...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 94.0 (PubMed & Canvas)", page_icon="🧬", layout="wide")
...
def buscar_en_pubmed(query, max_results=4):
    """Busca en PubMed vía API NCBI y devuelve resúmenes para el prompt"""
    try:
...
def procesar_pocus_nivel10(video_path):
    # (El motor POCUS se mantiene intacto)
...
def extraer_y_dibujar_bboxes(texto, img_pil=None):
    ...
    for match in matches:
        ...
        try:
            ...
            # Caja de patología en rojo brillante con mayor grosor
...
def analisis_avanzado_heridas(pil_image, usar_moneda=False):
    ...
    area_pixels_total = 0; best_contour = None
    for c in contours:
        if cv2.contourArea(c) > 500:
            area_pixels_total += cv2.contourArea(c)
            if best_contour is None or cv2.contourArea(c) > cv2.contourArea(best_contour): best_contour = c
    ...
    return {
        "area": area_final, "p_nec": p_nec, "p_esf": p_esf, "p_gra": p_gra,
        "img_calibrada": Image.fromarray(cv2.cvtColor(img_calibrada, cv2.COLOR_BGR2RGB)),
        "img_segmentada": Image.fromarray(cv2.cvtColor(img_segmentada, COLOR_BGR2RGB))
    }
...
st.title("🩺 LabMind 94.0 (Agente PubMed & Pizarra)")
...
with col_center:
    ...
    if modo == "📚 Agente Investigador (PubMed)":
        ...
        # --- PIZARRA INTERACTIVA PARA LA PRIMERA IMAGEN ---
    ...
    if btn_analizar:
        ...
        with st.spinner("🧠 Computando y consultando bases de datos..."):
            try:
                ...
                # --- PROCESAMIENTO DE IMÁGENES Y DIBUJOS ---
                    ...
                    for tipo, file in archivos:
                        if tipo == "doc":
                            ...
                            if imagen_dibujada is not None:
                                ...
                                imagen_para_visor = imagen_dibujada
                                imagen_dibujada = None # Solo añadirla una vez
                            else:
                ...
                if "ECG" in modo:
                    ...
                    instruccion_modo = f'Actúas como un investigador académico. He buscado en PubMed los artículos más recientes sobre la consulta del usuario. Tu trabajo es leer estos abstracts reales y redactar un informe clínico riguroso.\n\nEVIDENCIA EXTRAÍDA DE PUBMED:\n{datos_pubmed}'
                else:
                ...
                2. Si el usuario ha hecho un dibujo en la foto, céntrate en responder qué hay dentro de ese dibujo o zona marcada.
                3. {instruccion_modo}
                4. {instruccion_bbox}
                5. REGLA DE OMISIÓN: Si faltan datos de laboratorio, NO menciones que faltan. Ignóralo.
                ...
                # GUILLOTINA: Eliminar introducciones indeseadas de Gemini
                if "<div" in texto_generado:
                    texto_generado = texto_generado[texto_generado.find("<div"):]

                # EXTRAER Y DIBUJAR CAJAS ROJAS SI LAS GENERÓ (EJ. PARA ECG)
                if imagen_para_visor:
                    ...
                    elif imagen_para_visor: st.session_state.img_marcada = imagen_para_visor
