...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 62.0 (Brand Match)", page_icon="🧬", layout="wide")
...
with col_center:
    ...
    with tab_analisis:
        ...
        if modo == "🧩 Integral (Analizar Todo)":
            ...
            # --- KEY DINÁMICA PARA EVITAR ERROR DUPLICATE ---
            key_fuente = f"rad_source_{modo.replace(' ','_')}"
            fuente_label = st.radio("Fuente:", ["📁 Archivo", "📸 WebCam"], horizontal=True, label_visibility="collapsed", index=idx_fuente, key=key_fuente)
                ...
                try:
                    ...
                    if final_proto_obj:
                        ...
                        if is_pdf:
                            ...
                            txt_proto = "[PROTOCOLO EN IMAGEN ADJUNTA - ANALIZAR MARCAS Y PAUTAS]"
                    ...
                    - RADIOLOGÍA: {txt_rad_desc if 'txt_rad_desc' in locals() else ''}
                    
                    INSTRUCCIONES DE SALIDA (ESTRICTO):
                    1. NO REPITAS los textos de entrada (Farmacia/Protocolo) fuera de las cajas.
                    2. En la sección "CURA", DEBES INTEGRAR los productos específicos (marcas comerciales) detectados en el PROTOCOLO o FARMACIA que coincidan con la necesidad de la herida (ej: Iruxol, Aquacel, Cavilon, etc.).
                    ...
                    <div class="material-box"><b>🛠️ CURA (Materiales del Protocolo):</b><br>[Lista exacta de productos a usar según protocolo/farmacia disponible]</div>
