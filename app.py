...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 64.0 (Stability Fix)", page_icon="🧬", layout="wide")
...
time.sleep(0.1) # Wait for cookies to load
...
with col_center:
    ...
    with tab_analisis:
        ...
        # MODO LIMPIO (SAFE STRING PARA KEYS)
        clean_mode_key = re.sub(r'[^a-zA-Z0-9]', '', modo) 
        ...
        if modo == "🧩 Integral (Analizar Todo)":
            ...
            st.write("📸 **Visual:**")
            mostrar_imagenes = st.checkbox("👁️ Mostrar Análisis Visual", value=default_visual, key=f"chk_vis_{clean_mode_key}")
            fuente_label = st.radio("Fuente:", ["📁 Archivo", "📸 WebCam"], horizontal=True, label_visibility="collapsed", index=idx_fuente, key=f"rad_src_{clean_mode_key}")
            ...
            # KEYS UNICAS PARA EVITAR DUPLICADOS
            usar_moneda = st.checkbox("🪙 Usar moneda de 1€ para medir", value=default_moneda, key=f"chk_mon_{clean_mode_key}")
            mostrar_imagenes = st.checkbox("👁️ Mostrar Análisis Visual (Biofilm/Térmica)", value=default_visual, key=f"chk_vis_{clean_mode_key}")
            
            # Guardar Cookies (Solo si cambia)
            ...
            with st.expander("⏮️ Ver Evolución", expanded=False):
                prev = st.file_uploader("Foto Previa", type=['jpg','png'], accept_multiple_files=True, key=f"prev_{clean_mode_key}")
                ...
                with c_b: 
                    st.write(""); st.write("")
                    if st.button("➕", key=f"btn_add_{clean_mode_key}"): st.session_state.historial_evolucion.append({"Fecha": d_m.strftime("%d/%m"), "Area": a_m})
                if prev: 
                    for p in prev: archivos.append(("prev_img", p))

            with st.expander("💊 Contexto (Opcional)", expanded=False):
                meds_files = st.file_uploader("Docs", accept_multiple_files=True, key=f"meds_{clean_mode_key}")
            ...
            if fuente_label == "📸 WebCam":
                ...
                if fs := st.file_uploader("Subir", type=['jpg','png','mp4','mov'], accept_multiple_files=True, key=f"up_{clean_mode_key}"):
                    for f in fs: archivos.append(("video" if "video" in f.type else "img", f))
        ...
        if st.button("🚀 ANALIZAR", type="primary"):
            ...
            with st.spinner(f"🧠 Analizando {modo}..."):
                try:
                    ...
                    # --- PROMPT REFORZADO PARA INTEGRACIÓN ---
                    prompt = f"""
