...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 63.0 (Stable & Smart)", page_icon="🧬", layout="wide")
...
with col_center:
    ...
    with tab_analisis:
        ...
        if modo == "🧩 Integral (Analizar Todo)":
            ...
            fuente_label = st.radio("Fuente:", ["📁 Archivo", "📸 WebCam"], horizontal=True, label_visibility="collapsed", index=idx_fuente, key="rad_source_int")
            ...
            # KEYS FIJAS PARA EVITAR ERROR DUPLICATE KEY AL RECARGAR COOKIES
            usar_moneda = st.checkbox("🪙 Usar moneda de 1€ para medir", value=default_moneda, key="chk_moneda_fixed")
            mostrar_imagenes = st.checkbox("👁️ Mostrar Análisis Visual (Biofilm/Térmica)", value=default_visual, key="chk_visual_fixed")
            ...
            # KEY FIJA
            fuente_label = st.radio("Fuente:", ["📁 Archivo", "📸 WebCam"], horizontal=True, label_visibility="collapsed", index=idx_fuente, key="rad_source_fixed")
