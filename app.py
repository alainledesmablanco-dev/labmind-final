...
st.markdown("""
<style>
    .block-container { padding-top: 0.5rem !important; padding-bottom: 5rem !important; } /* Espacio extra para que no tape el chat flotante */
...
with col_center:
    ...
    if btn_analizar:
        ...
        with st.spinner("🧠 Computando y consultando bases de datos..."):
            try:
                ...
                prompt = f"""
                Rol: Médico Especialista IA V102. Contexto: {contexto}. Modo: {modo}.
    ...
    # RENDERIZADO DEL DIAGNÓSTICO EN LA COLUMNA CENTRAL
    if st.session_state.resultado_analisis:
        ...
        st.caption("Los mensajes de tu conversación con la IA aparecerán aquí.")
        
        # Aquí se imprimen los globos de chat para que no se desmadre la interfaz
...
# (Al ponerlo fuera de las columnas, Streamlit lo ancla a la base de la pantalla)
# ==========================================
if st.session_state.resultado_analisis:
    if user_query := st.chat_input("💬 Pregunta dudas, pide dosis o debata el diagnóstico aquí..."):
        # 1. Guardamos la pregunta del usuario
        st.session_state.chat_messages.append({"role": "user", "content": user_query})
        
        # 2. Consultamos a la IA en segundo plano
        try:
            genai.configure(api_key=st.session_state.api_key)
            chat_model = genai.GenerativeModel(st.session_state.modelo_seleccionado)
            
            # Contexto invisible: Le recordamos a la IA qué diagnóstico acaba de dar
            ctx_chat = f"Eres el médico experto de guardia. Has generado el siguiente informe previo:\n{st.session_state.resultado_analisis}\n\nResponde de forma clínica, concisa y directa a la siguiente duda del usuario:\n{user_query}"
            ...
            st.session_state.chat_messages.append({"role": "assistant", "content": f"⚠️ Hubo un error de conexión al consultar a la IA: {e}"})
        
        # 3. Forzamos una recarga rápida para que los mensajes aparezcan instantáneamente en la columna central
        st.rerun()
