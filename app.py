...
for key in ["autenticado", "api_key", "api_key_github", "resultado_analisis", "pdf_bytes", "chat_messages", "img_marcada", "video_bytes", "modelos_disponibles"]:
...
cookie_api_key_github = cookie_manager.get(cookie="labmind_github_key")

if not st.session_state.autenticado:
    if cookie_api_key:
        st.session_state.api_key = cookie_api_key
        if cookie_api_key_github:
            st.session_state.api_key_github = cookie_api_key_github
        ...
        if st.button("Entrar", type="primary"):
            if k:
                ...
                if k_gh:
                    cookie_manager.set("labmind_github_key", k_gh, expires_at=expires, key="set_cookie_github")
                    st.session_state.api_key_github = k_gh
                st.session_state.autenticado = True
                time.sleep(0.5)
                st.rerun()
            else:
...
"""
                ...
                if st.session_state.get("api_key_github") and st.session_state.get("modelo_gh"):
                    ...
                    try:
                        ...
                        TU MISIÓN ESTRICTA:
                        1. Observa la imagen adjunta (si la hay). ¿El "Diagnóstico" propuesto coincide EXACTAMENTE con lo que ves?
                        2. Si el borrador describe algo que NO está en la foto (alucinación visual), CORRIGE el texto, baja la Certeza y añade un aviso del error.
                        3. Tienes ESTRICTAMENTE PROHIBIDO presentarte, explicar tus acciones, o borrar las etiquetas HTML de diseño. Tu respuesta debe ser ÚNICAMENTE el código HTML.
                        4. Si es lógicamente impecable, devuelve el HTML EXACTAMENTE igual, añadiendo al título de la primera tarjeta: " 👁️ <b>[Auditado por {st.session_state.modelo_gh[:12]}]</b>".
                        """

                        message_content = [{"type": "text", "text": prompt_auditor}]
