...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 31.0 (Fixed Map)", page_icon="🧬", layout="wide")
...
# --- CARGADOR DE IMAGEN BLINDADO ---
@st.cache_data
def cargar_imagen_cuerpo_segura():
    # URL de Wikimedia (Permite hotlinking y es estable)
    url = "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ee/Human_body_schemes.svg/400px-Human_body_schemes.svg.png"
    try:
        response = requests.get(url, timeout=5)
        response.raise_for_status()
        return Image.open(BytesIO(response.content))
    except Exception:
        # Si falla, creamos una imagen de respaldo generada por código (nunca rota)
        img = Image.new('RGB', (400, 400), color=(240, 240, 240))
        d = ImageDraw.Draw(img)
        d.text((100, 180), "Imagen no disponible.\nUsa selector manual.", fill=(0, 0, 0))
        d.rectangle([10, 10, 390, 390], outline="red", width=3)
        return img
...
st.title("🩺 LabMind 31.0 (Fixed Images)")
...
with col_left:
    ...
    # Cargamos la imagen de forma segura (URL robusta + Fallback generado)
    img_body = cargar_imagen_cuerpo_segura()
    
    st.caption("Toca la zona afectada (Frontal/Dorsal):")
    value = streamlit_image_coordinates(img_body, key="body", height=400)
    
    if value:
        st.session_state.punto_cuerpo = value
        st.success("✅ Zona marcada correctamente")
    
    # Selector de respaldo siempre disponible por si el usuario prefiere lista
    zona_backup = st.selectbox("O selección manual:", 
