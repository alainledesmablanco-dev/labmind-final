...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 39.0 (Integral Max)", page_icon="🧬", layout="wide")
...
st.title("🩺 LabMind 39.0 (Integral Max)")
...
with col_center:
    ...
    archivos = []
    meds_files = None
    labs_files = None # Analíticas
    reports_files = None # Informes
    ecg_files = None 
    rad_files = None # RX/TAC en integral
    usar_moneda = False
    
    # === MODO INTEGRAL (RENOVADO) ===
    if modo == "🧩 Integral (Analizar Todo)":
        st.info("🧩 **Modo Integral**: Sube cualquier evidencia.")
        
        # PANEL DE CONTROL DOCUMENTAL (PLEGADO)
        with st.expander("📂 Documentación Clínica (Desplegar)", expanded=False):
            st.markdown("**1. Documentos de Texto / Datos:**")
            ...
            st.markdown("---")
            st.markdown("**2. Pruebas Diagnósticas (Imagen/Trazado):**")
            c4, c5 = st.columns(2)
            ecg_files = c4.file_uploader("📈 ECG", accept_multiple_files=True, key="int_ecg")
            rad_files = c5.file_uploader("💀 RX/TAC/RMN", accept_multiple_files=True, key="int_rad")
    ...
    if st.button("🚀 ANALIZAR", type="primary"):
        ...
        with st.spinner(f"🧠 Analizando {modo}..."):
            try:
                ...
                # 2. Documentos Texto (Loop genérico)
                for file_list, var_name in [(meds_files, "txt_meds"), (labs_files, "txt_labs"), (reports_files, "txt_reports")]:
                    if file_list:
                        ...
                        for f in file_list:
                            ...
                            else: con.append(Image.open(f)) # Si es foto de un papel
                        
                        # Asignar a variable correcta
                ...
                # 3. ECG (Tratamiento especial)
                if ecg_files:
                    for f in ecg_files:
                        if "pdf" in f.type: 
                            r = pypdf.PdfReader(f); txt_reports += "\n[ECG PDF TEXTO]: " + "".join([p.extract_text() for p in r.pages])
                        else: 
                            con.append(Image.open(f))
                            txt_reports += "\n[SE ADJUNTA IMAGEN DE ELECTROCARDIOGRAMA PARA ANALIZAR]"

                # 4. Radiología (Integral)
                if rad_files:
                    for f in rad_files:
                        if "pdf" in f.type: # Informe radiologico
                            r = pypdf.PdfReader(f); txt_reports += "\n[INFORME RX]: " + "".join([p.extract_text() for p in r.pages])
                        elif "video" in f.type: # Video scroll
                ...
                # Procesar Visual Principal (Paciente)
                ...
                - 💊 FÁRMACOS: {txt_meds}
                - 📊 ANALÍTICAS: {txt_labs}
                - 📄 INFORMES/HISTORIA: {txt_reports}
                - 💀 RADIOLOGÍA (Contexto): {txt_rad_desc}
                - 📸 VISUAL: {len(archivos)} archivos visuales principales.
                
                INSTRUCCIONES CLAVE:
                1. MODO INTEGRAL: Tu misión es la SÍNTESIS. Cruza los fármacos con las analíticas (ej: función renal vs dosis) y con lo que ves en las imágenes.
                ...
                \nOUTPUT:
                ### ⚡ RESUMEN CLÍNICO
                SYNC_ALERT: [ALERTA SI HAY CONFLICTO ENTRE PRUEBAS]
