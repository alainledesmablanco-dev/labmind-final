...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 95.0 (Blindada)", page_icon="🧬", layout="wide")
...
with col_center:
    ...
    if modo == "📚 Agente Investigador (PubMed)":
        ...
        # --- PIZARRA INTERACTIVA ULTRA-BLINDADA ---
            ...
            if habilitar_dibujo:
                ...
                try:
                    canvas_result = st_canvas(
                        ...
                        background_image=img_fondo_canvas, 
                    ...
                    if canvas_result.image_data is not None and np.sum(canvas_result.image_data) > 0:
                        ...
                        st.success("✅ Dibujo fusionado con éxito.")
                except Exception as e:
                    st.error("Error en la pizarra. Reinicia la app desde el panel general de Streamlit.")
        ...
        with st.expander("🎙️ Adjuntar Nota de Voz", expanded=False):
            try:
                # Intento forzado y protegido. Si falla, lo captura el except.
                audio_val = st.audio_input("Dictar notas", label_visibility="collapsed")
            except Exception:
                st.warning("⚠️ Tu servidor en la nube usa una versión antigua. Por favor, reinicia la app desde el panel principal (share.streamlit.io).")
                audio_val = None
    ...
    if btn_analizar:
        ...
        with st.spinner("🧠 Computando y consultando bases de datos..."):
            try:
                ...
                prompt = f"""
                Rol: Médico Especialista IA V95. Contexto: {contexto}. Modo: {modo}.
