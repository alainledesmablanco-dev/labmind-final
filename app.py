    ...
    if btn_analizar:
        ...
        with st.spinner("Procesando datos (Razonamiento en Cadena activado)..."):
            try:
                ...
                # --- LÓGICA DE TARJETAS Y PROTOCOLO SAN ELOY CON FLEXIBILIDAD CLÍNICA (V135) ---
                instruccion_extra_heridas = ""
                html_requerido = ""
                ...
                if modo == "🩹 Heridas / Úlceras":
                    try:
                        if os.path.exists("protocolo.jpg"):
                            ...
                            instruccion_extra_heridas = "- INSTRUCCIÓN DE CURAS: Te he adjuntado una foto del 'Protocolo de San Eloy'. PRIORIDAD MÁXIMA: Si vas a recomendar un apósito, material o crema que esté en el protocolo, usa EXACTAMENTE los nombres que aparecen en esa imagen. FLEXIBILIDAD: Si consideras que la herida requiere un producto o tratamiento que NO figura en el protocolo, tienes libertad para recomendarlo libremente como medida adicional basada en tu criterio clínico."
                    except Exception as e:
                        print(f"Error cargando protocolo.jpg: {e}")
                    ...
                    <details class="material-box" open><summary>🩹 CURA Y CUIDADOS</summary><p>[1. PAUTA DE CURA: Prioriza los materiales del Protocolo de San Eloy adjunto, pero añade otras recomendaciones si lo ves clínicamente necesario. 2. CUIDADOS DE ENFERMERÍA: Posición, monitorización, etc.]</p></details>
                    """
                else:
                    html_requerido = f"""
                    <details class="diagnosis-box" open><summary>🚨 HALLAZGOS Y RAZONAMIENTO</summary><p><b>[Diagnóstico]</b>. [Tu análisis]</p></details>
