...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LabMind 29.0", page_icon="🧬", layout="wide")

# --- IMAGEN CUERPO (BASE64 PARA QUE NO FALLE NUNCA) ---
# Esto asegura que la imagen cargue siempre sin depender de internet/links externos
def get_body_image_base64():
    # URL de respaldo por si acaso, pero usaremos un placeholder generado si falla o una url muy estable
    return "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/Human_body_features.svg/400px-Human_body_features.svg.png"
...
<style>
    ...
    [data-testid='stFileUploaderDropzone'] div div::after { content: "Arrastra Archivos Aquí"; font-size: 1rem; font-weight: bold; color: #444; display: block; }
</style>
...
def medir_herida(pil_image):
    try:
        ...
        if circles is not None:
            ...
            area_px = np.pi * (r_c**2) # Aprox simple para el ejemplo
...
st.title("🩺 LabMind 29.0 (Separated Modes)")
col_left, col_center, col_right = st.columns([1, 2, 1])

# --- COLUMNA 1: MAPA CORPORAL ---
with col_left:
    st.subheader("📍 Mapa Interactivo")
    # Usamos una URL muy estable
    body_url = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/Human_body_features.svg/300px-Human_body_features.svg.png"
    
    st.caption("Toca la zona afectada:")
    value = streamlit_image_coordinates(body_url, key="body", height=450)
    
    if value:
        st.session_state.punto_cuerpo = value
        st.success("✅ Zona Registrada")
    else:
        st.info("👆 Toca en la imagen")
...
with col_center:
    ...
    modo = st.selectbox("Selecciona Especialidad:", 
                 ["🧩 Integral (Todo)", 
                  "🩹 Heridas / Úlceras", 
                  "🧴 Dermatología", 
                  ...
                  "💀 RX / TAC / RMN (Imagen)", 
                  "📂 Analizar Informes (Docs)"])
    
    contexto = st.selectbox("🏥 Contexto:", ["Hospitalización", "Residencia", "Urgencias", "UCI", "Domicilio"])
    
    st.markdown("---")
    
    # --- LOGICA DE UPLOADERS SEGÚN MODO ---
    archivos = []
    meds_files = None
    tests_files = None
    
    # 1. MODO FARMACIA (Específico)
    if modo == "💊 Farmacia":
        st.info("📸 Sube foto de la **RECETA (Papel/Electrónica)** o la **CAJA/BLISTER** del medicamento.")
        meds_files = st.file_uploader("Foto Receta / Caja", accept_multiple_files=True, key="pharma_main")
    
    # 2. MODO RX/TAC (Específico)
    elif modo == "💀 RX / TAC / RMN":
        st.info("🎥 Sube **Video (Scroll)** de TAC/RMN o **Foto** de Placa RX.")
        if fs := st.file_uploader("Video o Imagen RX", type=['jpg','png','mp4','mov'], accept_multiple_files=True, key="rx_main"):
            for f in fs:
                ftype = "video" if "video" in f.type else "img"
                archivos.append((ftype, f))
                
    # 3. MODO ECG (Específico)
    elif modo == "📈 ECG":
        st.info("📈 Sube foto del **Electrocardiograma** (Papel o Pantalla).")
        if fs := st.file_uploader("Foto ECG", type=['jpg','png','pdf'], accept_multiple_files=True, key="ecg_main"):
            for f in fs: archivos.append(("img", f))

    # 4. MODO INFORMES (Solo Docs)
    elif modo == "📂 Analizar Informes":
        st.info("📄 Sube **Informes de Alta, Historia Clínica o Analíticas** para resumen.")
        tests_files = st.file_uploader("PDFs o Fotos de Documentos", accept_multiple_files=True, key="reports_main")

    # 5. RESTO DE MODOS (Heridas, Derma, Integral)
    else:
        # Uploaders Auxiliares (Opcionales para contexto)
        with st.expander("➕ Añadir Contexto (Fármacos / Analíticas)", expanded=False):
            c1, c2 = st.columns(2)
            meds_files = c1.file_uploader("💊 Fármacos", accept_multiple_files=True, key="aux_meds")
            tests_files = c2.file_uploader("📊 Analíticas", accept_multiple_files=True, key="aux_tests")

        # Uploader Principal
        st.write("📸 **Evidencia Visual (Lesión):**")
        ...
        if fuente == "📸 WebCam":
            ...
            if fs := st.file_uploader("Subir Foto/Video", type=['jpg','png','mp4','mov'], accept_multiple_files=True, key="std_main"):
                for f in fs:
                    ftype = "video" if "video" in f.type else "img"
                    archivos.append((ftype, f))
    ...
    if st.button("🚀 ANALIZAR", type="primary"):
        ...
        with st.spinner(f"🧠 Analizando modo: {modo}..."):
            try:
                ...
                # Prompt Dinámico según Especialidad
                coord_txt = ""
                if st.session_state.punto_cuerpo:
                    coord_txt = f"El usuario tocó en mapa X={st.session_state.punto_cuerpo['x']}, Y={st.session_state.punto_cuerpo['y']}."

                prompt = f"""
                Rol: APN / Especialista en {modo}. Contexto: {contexto}.
                {coord_txt}
                Notas: "{notas}"
                
                INPUTS:
                - FARMACIA (Txt/Img): {txt_meds}
                - INFORMES/PRUEBAS (Txt/Img): {txt_tests}
                ...
                if "Farmacia" in modo:
                    prompt += " IDENTIFICA MEDICAMENTOS (Caja/Receta). Busca interacciones, duplicidades y alergias. Alerta si dosis incorrecta."
                elif "RX" in modo:
                    prompt += " ACTÚA COMO RADIÓLOGO. Describe hallazgos en la imagen/video (fracturas, consolidaciones, nódulos). Scroll de video detecta profundidad."
                elif "ECG" in modo:
                    prompt += " ACTÚA COMO CARDIÓLOGO. Analiza ritmo, segmento ST, intervalos QT/PR. Busca arritmias o isquemia."
                elif "Informes" in modo:
                    prompt += " RESUME HISTORIA CLÍNICA. Extrae antecedentes, alergias y plan actual. Crea cronología."
                elif "Heridas" in modo:
                    prompt += " ANALIZA HERIDA + TERMOGRAFÍA + BIOFILM. Genera HTML BARRA TEJIDOS (sin markdown)."
                else:
                    prompt += " ANÁLISIS INTEGRAL. Cruza todos los datos."
