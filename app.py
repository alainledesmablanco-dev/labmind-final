...
def extraer_y_dibujar_bboxes(texto, img_pil):
    ...
    # --- FIX V136: CAJA ÚNICA ESTRICTA ---
    # Solo tomamos el PRIMER match absoluto para que no dibuje dos veces el mismo BBOX
    m = matches[0] 
    img_cv = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
    h, w = img_cv.shape[:2]
                ...
                if modo == "🩹 Heridas / Úlceras":
                    try:
                        if os.path.exists("protocolo.jpg"):
                            ...
                            instruccion_extra_heridas = "- INSTRUCCIÓN DE CURAS: Te he adjuntado una foto del 'Protocolo de San Eloy'. PRIORIDAD MÁXIMA: Si vas a recomendar un apósito, usa EXACTAMENTE los nombres de esa imagen. FLEXIBILIDAD: Si consideras que necesita algo que no está ahí, tienes libertad para añadirlo."
                    except Exception as e:
                        print(f"Error cargando protocolo.jpg: {e}")
                    
                    # --- FIX V136: HTML BLINDADO PARA HERIDAS (Saltos de línea forzados) ---
                    html_requerido = """
...
"""
                    ...
                    # --- FIX V136: HTML BLINDADO RESTO DE MODOS ---
                    html_requerido = f"""
<details class="diagnosis-box" open>
<summary>🚨 HALLAZGOS Y RAZONAMIENTO</summary>
<p><b>[Diagnóstico]</b>. [Tu análisis]</p>
...
"""
                ...
                # --- FIX V136: LIMPIEZA DE ARTEFACTOS MARKDOWN ---
